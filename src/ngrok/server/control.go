...
func (c *Control) registerTunnel(rawTunnelReq *msg.ReqTunnel) {
	for _, proto := range strings.Split(rawTunnelReq.Protocol, "+") {
		...
		authtoken, err := base64.StdEncoding.DecodeString(c.auth.User)
		if err != nil {
		...
		port, err := AesDecrypt(authtoken, key)
                if err != nil {
                        c.out <- &msg.NewTunnel{Error: err.Error()}
                        if len(c.tunnels) == 0 {
                                c.shutdown.Begin()
		...
		tunnelReq.RemotePort = uint16(iport)
		c.conn.Debug("Registering new tunnel ----> " + string(port))
